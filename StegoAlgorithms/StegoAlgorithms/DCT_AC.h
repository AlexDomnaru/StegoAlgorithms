...
cv::Mat DCT_AC(cv::Mat coverImage, string secret) {
	cv::Mat stegoImage;
...
void DCT_ACDisembed(cv::Mat stegoImage, size_t size) {
...
cv::Mat DCT_AC(cv::Mat coverImage, cv::Mat secret) {
	cv::Mat stegoImage;
...
void DCT_ACDisembed(cv::Mat stegoImage, int rows, int cols) {
	cv::Mat block, dctBlock;
